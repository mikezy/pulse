"""Happy-path tests for collectors.claude (all-time totals + 4-week heatmap)."""
import shutil
...
def test_collect_all_time_aggregates(tmp_path, monkeypatch):
    """All 5 fixture rows are counted in all-time totals."""
    projects = _setup_fake_projects(tmp_path)
    monkeypatch.setattr(claude, "CLAUDE_PROJECTS_DIR", projects)
    ...
    assert result["sessions_all"] == 4
    assert result["messages_all"] == 5
    assert result["tokens_all"] == 1550
    assert result["active_days_all"] == 3   # 2026-05-20, -21, -22
...
def test_collect_when_projects_dir_missing(tmp_path, monkeypatch):
    ...
    result = claude.collect()
    assert result["sessions_all"] == 0
    assert result["messages_all"] == 0
    assert result["tokens_all"] == 0
